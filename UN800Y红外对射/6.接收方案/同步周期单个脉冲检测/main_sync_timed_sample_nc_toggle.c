 ...
 *
 *  3. 同步后定时采样接收信号。
 *     上电、丢光后、同步周期到达后，先连续扫描 ADC。
 *     连续捕捉到 2 次有效脉冲后，认为已经和发射端 1ms 周期对齐。
 *     对齐后不再一直扫描 ADC，而是每隔 RX_TIMED_SAMPLE_INTERVAL_US
 *     采样一次接收信号，采样点落在下一次有效脉冲附近。
 *
 ...
 * 注意：
 *  这个宏只决定“什么时候进入定时采样模式”；
 *  原来的输出确认次数仍然由 RX_PULSE_CONFIRM_COUNT 决定。
 */
#define RX_SYNC_LOCK_COUNT              2U

/*
 * 定时采样间隔。
 *
 * 发射端正常有效脉冲周期约 1ms。
 * 因为实际 1ms 不一定非常准，所以这里做成宏，现场可微调：
 *  1000 -> 1.000ms
 *  990  -> 0.990ms
 *  1010 -> 1.010ms
 *
 * 如果发现进入定时采样后偶尔采不到脉冲：
 *  1. 先用示波器确认发射端真实周期；
 *  2. 再微调 RX_TIMED_SAMPLE_INTERVAL_US。
 */
#define RX_TIMED_SAMPLE_INTERVAL_US     1000U
#define RX_TIMED_SAMPLE_INTERVAL_TICKS  TIMER0_US_TO_TICKS(RX_TIMED_SAMPLE_INTERVAL_US)

/*
 * 重新同步周期。
 *
 * 同步只能保证一段时间内采样点准确。
 * 所以定时采样运行一段时间后，主动回到连续扫描模式，
 * 再捕捉 2 次有效脉冲，重新校准采样相位。
 *
 * 默认 100ms 同步一次。
 */
#define RX_RESYNC_PERIOD_MS             100U
#define RX_RESYNC_PERIOD_TICKS          TIMER0_MS_TO_TICKS(RX_RESYNC_PERIOD_MS)

/*
 * 定时采样丢失次数。
 *
 * 在定时采样模式下，如果某次采样没有采到有效脉冲，
 * 说明相位可能已经偏了，或者光被遮挡。
 *
 * 这里默认 1 次没采到就立即回到连续扫描模式重新同步。
 * 注意不会立刻改变输出，输出仍由原来的 RX_LOST_TIMEOUT_MS 控制。
 */
#define RX_TIMED_SAMPLE_MISS_MAX        1U

/*
 * 接收状态机模式。
 */
#define RX_MODE_SYNC_SCAN               0U
#define RX_MODE_TIMED_SAMPLE            1U

/*
 * NC 采样点调试输出。
 *
 * 1：进入定时采样模式后，每到一次采样点，就翻转一次 NC 输出。
 *    示波器看 P1.2 / NC，就能判断采样点是否和接收脉冲对齐。
 *
 * 0：关闭该调试功能。
 *
 * 注意：
 *  当前版本把 NC 当作调试脚使用，output_apply() 里不会再控制 NC。
 */
#define RX_SAMPLE_TOGGLE_NC_DEBUG       1U
 ...
 *
 * RX_MODE_SYNC_SCAN：
 *  连续扫描 ADC，用于捕捉两个有效脉冲并完成同步。
 *
 * RX_MODE_TIMED_SAMPLE：
 *  已同步，按 1ms 左右间隔定时采样 ADC。
 */
static uint8_t g_rx_mode = RX_MODE_SYNC_SCAN;

/*
 * 本轮同步已经捕捉到的有效脉冲数量。
 *
 * 只用于“同步锁定”，不改变原来的输出判断逻辑。
 */
 ...
 * 定时采样模式下，连续采样失败次数。
 */
static uint8_t g_timed_sample_miss_count = 0;
 ...
 * 单位：10us tick。
 */
static uint16_t g_next_sample_tick = 0;
 ...
 * 用于判断什么时候需要周期性重新同步。
 */
static uint16_t g_sync_lock_tick = 0;

/*
 * NC 调试输出当前状态。
 * 0：输出 OUT_INACTIVE_LEVEL
 * 1：输出 OUT_ACTIVE_LEVEL
 */
static uint8_t g_nc_sample_toggle_state = 0;
...
static uint8_t tick_reached(uint16_t now, uint16_t target);

static void signal_receive_process(void);
static void signal_fast_scan_process(void);
static void signal_timed_sample_process(void);
static void rx_enter_sync_scan_mode(void);
static void rx_enter_timed_sample_mode(void);
static void sync_period_process(void);
...
static void nc_sample_debug_reset(void);
static void nc_sample_debug_toggle(void);
...
void main(void)
{
    ...
    /*
     * 当前 NC 被用作采样点调试输出，上电后先复位到固定电平。
     */
    nc_sample_debug_reset();

    /*
     * 主循环。
     *
     * 接收流程：
     *  1. 未同步时，连续快速扫描 ADC，捕捉 2 个有效脉冲；
     *  2. 同步后，按 RX_TIMED_SAMPLE_INTERVAL_US 定时采样；
     *  3. 定时采样失败或同步周期到达后，回到连续扫描重新同步。
     */
    while(1)
    {
        /*
...
/*
 * 判断 now 是否已经到达 target。
 *
 * 这里使用 uint16_t 自然回绕比较。
 * 只要比较的时间跨度小于 32768 个 tick，也就是 327.68ms，
 * 就能正确处理 g_tick_10us 回绕。
 */
static uint8_t tick_reached(uint16_t now, uint16_t target)
{
    uint16_t diff;

    diff = (uint16_t)(now - target);
 ...
 *
 * 已同步：
 *  不再一直扫 ADC，只在预计有效脉冲到来时采样一次。
 */
static void signal_receive_process(void)
{
    if(g_rx_mode == RX_MODE_TIMED_SAMPLE)
    {
        signal_timed_sample_process();
    }
    else
    {
...
static void rx_enter_sync_scan_mode(void)
{
    g_rx_mode = RX_MODE_SYNC_SCAN;
    g_sync_pulse_count = 0;
    g_timed_sample_miss_count = 0;
    g_in_pulse = 0;

    /*
     * 回到扫描模式时复位 NC 调试脚，避免调试波形停在不确定状态。
     */
    nc_sample_debug_reset();
}
 ...
 * 进入定时采样模式。
 *
 * 当前时刻已经捕捉到第 RX_SYNC_LOCK_COUNT 个有效脉冲。
 * 因为后续有效脉冲约每 1ms 出现一次，
 * 所以下一次采样点设置为最近一次有效脉冲时间 + 采样间隔。
 */
static void rx_enter_timed_sample_mode(void)
{
    g_rx_mode = RX_MODE_TIMED_SAMPLE;
    g_timed_sample_miss_count = 0;
    g_in_pulse = 0;

    g_sync_lock_tick = g_tick_10us;
    g_next_sample_tick = (uint16_t)(g_last_pulse_tick + RX_TIMED_SAMPLE_INTERVAL_TICKS);

    /*
     * 进入定时采样前，先把 NC 调试脚拉回固定初始状态。
     * 后续每到一次采样点，就翻转一次。
     */
    nc_sample_debug_reset();
}
 ...
 * 同步周期处理。
 *
 * 定时采样运行 RX_RESYNC_PERIOD_MS 后，
 * 主动回到连续扫描模式重新同步。
 *
 * 这样可以避免 1ms 周期轻微误差长时间累积后，
 * 采样点逐渐偏离有效脉冲。
 */
static void sync_period_process(void)
{
    ...
    diff = (uint16_t)(g_tick_10us - g_sync_lock_tick);

    if(diff >= RX_RESYNC_PERIOD_TICKS)
    {
        rx_enter_sync_scan_mode();
    }
}
...
static void signal_fast_scan_process(void)
{
    ...
    if(g_in_pulse == 0)
    {
        ...
        if(signal_enter_active(signal))
        {
            ...
            /*
             * 连续扫描阶段不翻转 NC，避免和定时采样点调试波形混在一起。
             */

            /*
             * 处理一次有效脉冲。
             *
             * 这里仍然使用原来的 valid_pulse_process()，
             * 所以输出确认和丢光控制逻辑不变。
             */
            valid_pulse_process();

            /*
             * 同步扫描模式下，连续捕捉到指定数量的有效脉冲后，
             * 进入定时采样模式。
             */
            if(g_rx_mode == RX_MODE_SYNC_SCAN)
            {
                ...
                if(g_sync_pulse_count >= RX_SYNC_LOCK_COUNT)
                {
                    rx_enter_timed_sample_mode();
                }
            }
        }
    }
 ...
 *
 * 同步锁定后，不再连续读取 ADC。
 * 只有到了 g_next_sample_tick，才读取一次接收信号 ADC。
 *
 * 如果采样成功：
 *  仍然调用 valid_pulse_process()，原来的输出逻辑不变。
 *
 * 如果采样失败：
 *  不马上改变输出，只回到连续扫描模式重新同步。
 *  输出是否变成遮光，仍由 lost_timeout_process() 按 RX_LOST_TIMEOUT_MS 决定。
 */
static void signal_timed_sample_process(void)
{
    uint16_t now;
     ...
     * 还没到下一次采样点，不占用 ADC。
     */
    if(tick_reached(now, g_next_sample_tick) == 0)
    {
        return;
    }
    ...
    if(signal_enter_active(signal))
    {
         ...
         *
         * 优先使用“上一次计划采样时间 + 周期”，
         * 这样不会因为本次 ADC 转换耗时产生累计漂移。
         */
        g_next_sample_tick = (uint16_t)(g_next_sample_tick + RX_TIMED_SAMPLE_INTERVAL_TICKS);

        /*
         * 如果因为某些慢任务导致当前已经错过下一次采样点，
         * 就从当前时间重新排下一次，避免连续补采样。
         */
        if(tick_reached(g_tick_10us, g_next_sample_tick))
        {
            g_next_sample_tick = (uint16_t)(g_tick_10us + RX_TIMED_SAMPLE_INTERVAL_TICKS);
        }
    }
    else
    {
        /*
         * 本次定时采样没采到有效脉冲。
         * 可能是 1ms 周期有偏差，也可能是已经遮光。
         */
        if(g_timed_sample_miss_count < 255U)
        {
            g_timed_sample_miss_count++;
        }

        if(g_timed_sample_miss_count >= RX_TIMED_SAMPLE_MISS_MAX)
        {
            /*
             * 只重新同步，不立即改变输出状态。
             */
...
static void nc_sample_debug_reset(void)
{
#if RX_SAMPLE_TOGGLE_NC_DEBUG
    g_nc_sample_toggle_state = 0;
...
{
#if RX_SAMPLE_TOGGLE_NC_DEBUG
    if(g_nc_sample_toggle_state == 0)
    {
        g_nc_sample_toggle_state = 1;
        gpio_io_set(IR_OUT_NC_PIN, OUT_ACTIVE_LEVEL);
    }
    else
    {
