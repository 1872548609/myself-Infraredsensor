 ...
 */
#define RX_WINDOW_INTERVAL_TICK       2700U
 ...
 */
#define RX_FIRST_WINDOW_DELAY_TICK    2400U
 ...
 * - 建议先用 30~60us 调试。
 */
#define RX_ADC_WINDOW_WIDTH_TICK      170U
 ...
 */
#define RX_SYNC_WAIT_TIMEOUT_TICK     3200U
 ...
 */
#define RX_STATUS_LED_ENABLE                    1U
#define RX_STATUS_LED_ACTIVE_LEVEL              GPIO_HIGH
#define RX_STATUS_LED_INACTIVE_LEVEL            GPIO_LOW

/*
 * 每累计多少个 ADC 窗口翻转一次指示灯。
 * 以 1ms 窗口为例：250U = 亮约 250ms、灭约 250ms，约 2Hz 闪烁。
 */
#define RX_STATUS_LED_BLINK_HALF_PERIOD_WINDOW  250U

/* P1_0 的 ADC 窗口示波器输出：窗口期间拉低，窗口结束恢复高电平。 */
#define RX_ADC_WINDOW_DEBUG_ENABLE              0U
...
/*
    A=591：窗口内 ADC 平均值为 591。
    TON=500：有光进入阈值。
    TOF=400：有光退出阈值。
    OK=1：591 > 500，本窗口判定有效。
    L=1：当前已切换到有光输出。
    N=6：100 μs 窗口内完成了 6 次 ADC 采样。
    DT=2400：本次窗口的软件记录周期延迟是 2400 μs。
...
/*
 * 迟滞区间提示状态：
 * rx_hys_zone_active = 1 表示最近一次 ADC 处于 (TOF, TON) 开区间。
 * 这只是“提示灯状态”，不参与任何输出判定，不改变 rx_adc_valid_state。
 */
static volatile uint8_t  rx_hys_zone_active = 0U;
static volatile uint16_t rx_led_blink_window_count = 0U;
static volatile uint8_t  rx_led_blink_on = 0U;
...
static void rx_status_led_set(uint8_t led_on);
static void rx_status_led_update(uint8_t hys_zone);
...
static uint8_t rx_adc_is_hysteresis_zone(uint16_t adc);
 ...
 * 设置 P1_3 状态指示灯。
 *
 * 该函数只操作 P1_3；P1_2 的实际检测输出完全不参与闪烁。
 * P1_3 沿用原逻辑：高电平点亮、低电平熄灭。
 */
static void rx_status_led_set(uint8_t led_on)
{
...
static void rx_status_led_update(uint8_t hys_zone)
{
    ...
    /*
     * 第一次进入迟滞区时，先亮灯再按周期翻转。
     * 这样即使当前输出保持遮光，也能马上看出“此处不稳定”。
     */
    if (rx_hys_zone_active == 0U)
    {
        rx_hys_zone_active        = 1U;
        rx_led_blink_window_count = 0U;
        rx_led_blink_on           = 1U;
    ...
    if (RX_STATUS_LED_BLINK_HALF_PERIOD_WINDOW == 0U)
    {
        return;
    }

    if (rx_led_blink_window_count < 65535U)
    {
        rx_led_blink_window_count++;
    }

    if (rx_led_blink_window_count >= RX_STATUS_LED_BLINK_HALF_PERIOD_WINDOW)
    {
        rx_led_blink_window_count = 0U;
        rx_led_blink_on = (uint8_t)(rx_led_blink_on == 0U);

        rx_status_led_set(rx_led_blink_on);
    }
}
 ...
 * - ADC >= TON：已经明确有效。
 *
 * 此函数不改变 rx_adc_valid_state，只给 P1_3 指示灯提供提示信息。
 */
static uint8_t rx_adc_is_hysteresis_zone(uint16_t adc)
{
    uint16_t adc_off_th;
...
static void rx_process_adc_window(void)
{
    ...
    /*
     * hys_zone 只用于告警灯：
     * ADC 落在 TOF < ADC < TON 时，灯闪烁，但 adc_ok 仍严格按原迟滞状态机决定。
     */
    hys_zone = rx_adc_is_hysteresis_zone(avg_adc);
    adc_ok   = rx_adc_hysteresis_check(avg_adc);//== 原有迟滞判断，绝不因闪灯而改变
#else
    hys_zone = 0U;
