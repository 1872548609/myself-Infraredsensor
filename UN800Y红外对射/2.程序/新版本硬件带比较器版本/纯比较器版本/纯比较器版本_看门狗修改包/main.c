...
/* GTIMER0 中断产生心跳，主循环消费心跳后才能喂狗 */
volatile uint8_t g_wdt_timer_heartbeat = 0;
...
void main(void)
{
    ...
    while (1)
    {
         ...
         */
        if (g_wdt_timer_heartbeat != 0)
        {
            g_wdt_timer_heartbeat = 0;
            wdt_feed(APP_WDT_TIMEOUT);
        }
    }
}
