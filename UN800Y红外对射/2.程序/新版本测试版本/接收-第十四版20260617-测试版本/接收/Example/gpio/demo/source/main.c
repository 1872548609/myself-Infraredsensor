...
/* GTIMER1：强制重同步等待 P1_4 边沿的 2ms 一次性超时定时器。 */
/*
...
static void rx_start_periodic_window_from_sync(void)
{
    ...
    rx_sync_timeout_timer_stop_clear();
 ...
 * 主循环里执行强制重同步。
 *
 * 这是当前版本“遮光偶尔不判断”的重点怀疑函数。
 *
 * 它做了几件事：
 * 1. rx_seen_once = 0：退出 ADC 周期采样状态。
 * 2. rx_timer_stop_clear()：停止 GTIMER0，后续不再产生 ADC 窗口。
 * 3. 打开 P1_4 同步中断，等待新边沿。
 * 4. 如果当前输出仍是有光，则启动 GTIMER1 2ms 超时。
 *
 * 风险：
 * 遮光刚好发生在这个阶段时，不会再经过连续 3 个 ADC 无效窗口，
 * 只能等 GTIMER1 超时处理。如果 GTIMER1 或主循环处理不及时，输出会保持有光。
 */
static void rx_force_resync_from_main(void)
{
    rx_resync_pending = 0U;
    ...
    /*
     * 这里退出“已同步 ADC 窗口采样状态”。
     * 从这一刻开始，GTIMER0 会被停掉，遮光不再走 ADC 连续无效窗口路径。
     */
    rx_seen_once = 0U;
    rx_window_pending = 0U;
    rx_sync_pending = 0U;
     ...
     * 也是遮光偶发不判断时最需要示波器确认的动作：
     * 如果 P1_0 ADC 窗口脚停止跳变，说明已经进入重同步等待期。
     */
    rx_timer_stop_clear();
    ...
    {
        /*
         * 只有当前已经输出有光，强制重同步才启动 2ms 超时。
         * 如果这里没启动，遮光时输出可能保持当前状态直到下一次事件。
         */
...
/*
 * 主循环上下文里的同步等待超时处理。
 * 如果强制重同步后 2ms 内没有收到 P1_4，同步等待超时，直接判定遮光。
 */
/*
 * 处理强制重同步期间的 2ms 超时。
 *
 ...
 *
 * 排查点：
 * 如果遮光不判断时 rx_light_state=1 且 rx_seen_once=0，
 * 但这个函数没有执行到 rx_enter_block_state，说明 GTIMER1 超时链路有问题。
 */
static void rx_handle_sync_wait_timeout_from_main(void)
{
 ...
 *
 * 重点问题：
 * RX_RESYNC_WINDOW_COUNT 当前是 3U，导致重同步太频繁。
 *
 * 另外：
 * 当前代码即使 rx_light_state 还没真正确认有光，也会累计重同步次数。
 * 更稳的做法是：只有 rx_light_state==1 且 adc_ok==1 才累计。
 */
static void rx_resync_count_after_window(uint8_t adc_ok)
{
#if RX_RESYNC_ENABLE
    ...
    if ((rx_resync_window_count >= RX_RESYNC_WINDOW_COUNT) && (adc_ok != 0U))
    {
        /*
         * 达到强制重同步条件。
         * 当前 RX_RESYNC_WINDOW_COUNT=3U，所以这里非常容易频繁触发。
         */
...
static void rx_process_adc_window(void)
{
    ...
    else
    {
        ...
        if (rx_lost_window_count < RX_LOST_WINDOW_COUNT)
        {
            /*
             * 正常遮光确认计数。
             * 连续无效窗口达到 RX_LOST_WINDOW_COUNT 才会真正拉低输出。
             */
            rx_lost_window_count++;
        }
...
void gtimer1_UECallBack(void)
{
    ...
    if (rx_sync_wait_timer_running != 0U)
    {
        rx_sync_wait_timer_running   = 0U;
        rx_sync_wait_timeout_pending = 1U;
    }
}
 ...
 * 【结尾排查建议】
 *
 * 如果你要定位“遮光有时候不判断”，建议按下面顺序观察：
 *
 * 1. 先看 P1_0 ADC 窗口脚：
 *    - 遮光时 P1_0 仍然每 1ms 跳变：说明还在正常 ADC 判断链路。
 *    - 遮光时 P1_0 停止跳变：说明进入了强制重同步等待链路。
 *
 * 2. 若 P1_0 仍跳变，看 adc_data：
 *    - adc_data 是否低于 adc_set - RX_ADC_HYS_VALUE。
 *    - 如果没有低于，迟滞会继续认为有效。
 *
 * 3. 若 P1_0 停止跳变，看 GTIMER1：
 *    - gtimer1_UECallBack() 是否进中断。
 *    - rx_sync_wait_timeout_pending 是否置位。
 *    - 主循环是否调用 rx_handle_sync_wait_timeout_from_main()。
 *
 * 4. 临时验证建议：
 *    - 把 RX_RESYNC_ENABLE 改成 0U。
 *    - 或把 RX_RESYNC_WINDOW_COUNT 改成 100U。
 *    如果问题消失，基本确定是强制重同步逻辑引起。
 ***********************************************************************/
