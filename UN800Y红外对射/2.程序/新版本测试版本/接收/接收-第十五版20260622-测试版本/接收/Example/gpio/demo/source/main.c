 ...
 */
#define RX_ADC_HYS_VALUE              500U  /* 原 100：第 1 点判定不再做 2 点平均，低阈值同步放大 2 倍 */
 ...
 */
/*
 * printfS() 很可能是阻塞式发送，不能在每个 1ms ADC 窗口中直接打印。
 * 本调试版本只保存本窗口快照，并只在停止窗口、等待下一次同步的空档打印。
 */
/*
 * ADC 窗口形状调试：
 * - 只在低频分频或输出状态变化时打印，避免串口影响采样。
 * - 命中阈值仅用于统计 HC/SPAN，不参与当前输出判定。
 * - 现场可根据日志把 RX_ADC_DEBUG_HIT_THRESHOLD 调到
 *   “无光背景以上、真实脉冲以下”的位置。
 */
#define RX_ADC_DEBUG_HIT_THRESHOLD         400U  
...
volatile uint16_t adc_data  = 0;      /* 最近一次 ADC 窗口第 1 个采样值：当前实际判定值 */
volatile uint16_t adc_data1 = 0;      /* 最近一次 ADC 窗口采样次数，方便调试 */

/*
 * 窗口形状调试量：
 * adc_peak          ：本窗口有效采样中的最大 ADC。
 * adc_peak_sample_index：峰值出现的第几个 ADC 采样点，1 起始。
 * adc_hit_count     ：ADC >= RX_ADC_DEBUG_HIT_THRESHOLD 的采样点数。
 * adc_hit_span_tick ：第一个命中点到最后一个命中点的时间跨度，单位 us。
 *                     值为 0 可能表示“没有命中”或“只命中一个采样点”。
 */
volatile uint16_t adc_peak              = 0;
volatile uint16_t adc_peak_sample_index = 0; /* 峰值出现的第几个 ADC 采样点，1 起始；0 表示本窗口无有效峰值 */
volatile uint16_t adc_hit_count         = 0;
volatile uint16_t adc_hit_span_tick     = 0;

/* 当前 ADC 有效阈值。现在使用窗口第 1 个采样点判定，阈值相对原平均值版本放大 2 倍。 */
volatile uint16_t adc_set = 1000;     /* 原 500：第 1 点判定不再除以样本数，开阈值放大 2 倍 */
...
/* 打印时使用快照，避免下一窗口覆盖刚刚要观察的数据。 */
static volatile uint16_t g_dbg_print_adc                = 0U;
static volatile uint16_t g_dbg_print_peak_adc           = 0U;
static volatile uint16_t g_dbg_print_peak_sample_index  = 0U;
static volatile uint16_t g_dbg_print_hit_count          = 0U;
static volatile uint16_t g_dbg_print_hit_span_tick = 0U;
...
static uint16_t rx_adc_scan_window_average(uint8_t ch,
                                           uint16_t window_tick,
                                           uint16_t *sample_count,
                                           uint16_t *peak_adc,
                                           uint16_t *peak_sample_index,
                                           uint16_t *hit_count,
                                           uint16_t *hit_span_tick);
 ...
 * 在固定窗口内连续读取 ADC。
 *
 * 当前判定值改为“窗口第 1 个 ADC 采样值”，不再使用 sum / count 平均值：
 * - 第 1 点与已对齐的真实发射脉冲峰值对应；
 * - 后续点继续采集，仅用于保留 PK / PKI / N 等调试信息；
 * - 若第 1 点为 ADC_INVALID，则整个窗口按无效处理。
 *
 * 这样能够避免两点采样时被第 2 个低值除以 2 造成的幅值折半。
 */
static uint16_t rx_adc_scan_window_average(uint8_t ch,
                                           uint16_t window_tick,
                                           uint16_t *sample_count,
                                           ...
                                           uint16_t *hit_span_tick)
{
    ...
    uint8_t  first_sample_seen;
    uint16_t hit_num;
    uint16_t first_hit_tick;
    uint16_t last_hit_tick;
    uint8_t  hit_seen;
    count          = 0U;
    sample_index   = 0U;
    peak           = 0U;
    peak_index     = 0U;
    first_sample   = ADC_INVALID;
    first_sample_seen = 0U;
    hit_num        = 0U;
    first_hit_tick = 0U;
    last_hit_tick  = 0U;
    hit_seen       = 0U;
    ...
    do
    {
        ...
        elapsed_tick = rx_timer_elapsed_from(start_tick);

        /* 第几个 ADC 采样点：从 1 开始计数，包含窗口内所有转换。 */
        if (sample_index < 65535U)
        {
            sample_index++;
        }

        /*
         * 当前输出判定只使用窗口内第 1 个 ADC 采样点。
         * 保留其原始值；若它饱和/无效，返回 ADC_INVALID，
         * 不能让后续采样点替代第 1 点误判为有光。
         */
        if (first_sample_seen == 0U)
        {
            first_sample      = value;
            first_sample_seen = 1U;
        }
        ...
        if (value < RX_ADC_MAX_VALID)
        {
             ...
             * 单点命中会显示 SPAN=0。
             */
            if (value >= RX_ADC_DEBUG_HIT_THRESHOLD)
            {
                if (hit_num < 65535U)
                {
                    hit_num++;
                ...
                if (hit_seen == 0U)
                {
                    first_hit_tick = elapsed_tick;
                    hit_seen = 1U;
                }

                last_hit_tick = elapsed_tick;
            }
        }
    }
    ...
    if (hit_span_tick != 0)
    {
        if (hit_seen != 0U)
        {
            *hit_span_tick = (uint16_t)(last_hit_tick - first_hit_tick);
        }
        else
        {
            *hit_span_tick = 0U;
    ...
    if ((count == 0U) || (first_sample_seen == 0U))
    {
        return ADC_INVALID;
    }
    ...
    /*
     * 不再返回窗口平均值。
     * 返回第 1 个采样点作为本窗口的实际判定值。
     */
    if (first_sample >= RX_ADC_MAX_VALID)
    {
        return ADC_INVALID;
    }
 ...
 *
 * 排查点：
 * 遮光后如果第 1 个 ADC 点没有降到 adc_set - RX_ADC_HYS_VALUE 以下，
...
static void rx_process_adc_window(void)
{
    ...
    sample_count      = 0U;
    peak_adc          = 0U;
    peak_sample_index = 0U;
    hit_count         = 0U;
    hit_span_tick = 0U;

    first_adc = rx_adc_scan_window_average(ADC_CHANNEL_1,
                                         RX_ADC_WINDOW_WIDTH_TICK,
                                         &sample_count,
                                         &peak_adc,
                                         &peak_sample_index,
                                         &hit_count,
                                         &hit_span_tick);//== 获取第 1 点判定值和窗口形状

    adc_data              = first_adc;//== 第 1 个 ADC 采样值，作为本窗口判定值
    adc_data1             = sample_count;//== 采样数量
    adc_peak              = peak_adc;//== 窗口峰值
    adc_peak_sample_index = peak_sample_index;//== 峰值出现的第几个 ADC 采样点（1 起始）
    adc_hit_count         = hit_count;//== 超过小阈值的采样点数
    adc_hit_span_tick = hit_span_tick;//== 首末命中跨度，单位us
    adcount           = sample_count;
    g_dbg_last_elapsed_tick = rx_last_trigger_interval_tick;//== 采样窗口间隔

#if RX_ADC_HYS_ENABLE
    adc_ok = rx_adc_hysteresis_check(adc_peak);//== 基于第 1 个 ADC 点做迟滞判断
#else
    if ((first_adc < RX_ADC_MAX_VALID) && (first_adc >= adc_set))
    {
        adc_ok = 1U;
    }
    else
...
#if UART_ADC_DEBUG_ENABLE
    /*
     * 每个窗口都只保存数据快照；一般按分频低频输出。
     * 输出状态变化时，强制保留触发本次切换的窗口。
     */
    if (adc_ok != 0U)
    {
...
static void uart_debug_request(uint8_t reason, uint8_t force)
{
    ...
    /*
     * 每个窗口只保存快照，不直接 printf。
     * 真正的输出由 uart_adc_debug_process() 在安全空档完成。
     */
    ...
    g_dbg_print_peak_adc          = adc_peak;
    g_dbg_print_peak_sample_index = adc_peak_sample_index;
    g_dbg_print_hit_count         = adc_hit_count;
    g_dbg_print_hit_span_tick = adc_hit_span_tick;
...
/*
 * 只有 ADC 周期窗口已停止、且没有待处理同步边沿时才发送串口。
 *
 * 输出前暂时关闭 P1_4 同步中断，避免 printfS() 阻塞期间捕获一个旧边沿，
 * 却在发送完成数毫秒后才拿它建立相位。发送完成后重新开中断，下一条边沿
 * 会作为新的真实同步点。
 */
static void uart_adc_debug_process(void)
{
    ...
    /*
     * P1  ：窗口第 1 个 ADC 采样值，也是当前实际判定值
     * PK  ：窗口内最大 ADC
     * PKI ：峰值出现在本窗口第几个 ADC 采样点；从 1 开始，0 表示无有效采样
     * TON ：迟滞进入有光的阈值 adc_set
     * TOF ：迟滞退出有光的阈值 adc_set - RX_ADC_HYS_VALUE
     * OK  ：当前窗口迟滞结果，1 有效，0 无效
     * L   ：当前输出状态，1 有光，0 遮光
     * N   ：本窗口内 ADC 有效采样点数
     */
    printfS("P1=%u,PK=%u,PKI=%u,TON=%u,TOF=%u,OK=%u,L=%u,N=%u\r\n",
            g_dbg_print_adc,
            g_dbg_print_peak_adc,
            g_dbg_print_peak_sample_index,
            g_dbg_print_th_on,
            g_dbg_print_th_off,
...
void main(void)
{
    ...
    while (1)
    {
        ...
        /*
         * 调试输出必须放在状态机之后。函数内部还会确认当前处于安全空档。
         */
