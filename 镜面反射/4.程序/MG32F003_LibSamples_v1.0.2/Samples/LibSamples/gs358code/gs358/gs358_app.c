...
void GS358_AppInit(void)
{
    ...
    /*
     * 接收端、窗口检测和超时定时器全部准备完成后，
     * 再根据宏配置启动 PA11 / TIM14_CH1 发射 PWM。
     */
    GS358_PWMInit();
...
static void GS358_PWMInit(void)
{
    ...
    timer_clock_hz = TIM_GetTIMxClock(GS358_PWM_TIMER);
    prescaler_div =
    ...
    timer_init.TIM_CounterMode = TIM_CounterMode_Up;
    timer_init.TIM_Period = period_ticks - 1UL;
    timer_init.TIM_ClockDivision = TIM_CKD_Div1;
    timer_init.TIM_RepetitionCounter = 0U;
    TIM_TimeBaseInit(GS358_PWM_TIMER, &timer_init);
    ...
    oc_init.TIM_OCPolarity =
        (GS358_PWM_ACTIVE_HIGH != 0U) ?
        TIM_OCPolarity_High : TIM_OCPolarity_Low;

    TIM_OC1Init(GS358_PWM_TIMER, &oc_init);
    TIM_OC1PreloadConfig(GS358_PWM_TIMER,
                         TIM_OCPreload_Enable);
    TIM_ARRPreloadConfig(GS358_PWM_TIMER, ENABLE);
