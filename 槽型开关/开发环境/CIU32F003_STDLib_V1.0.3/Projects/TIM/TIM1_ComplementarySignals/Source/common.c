...
/*--------------------------------------------define--------------------------------------------*/
#define  TIM_PERIOD_VALUE       0xFFFF
...
#define  TIM_DEAD_TIME_VALUE    0x3F
...
void tim1_init(void)
{
    ...
    /* 配置通道1输出模式为PWM1模式 */
    output_init.output_compare_mode = TIM_OUTPUT_MODE_PWM1;
    ...
    break_config.dead_time = TIM_DEAD_TIME_VALUE;
...
void gpio_init(void)
{
    ...
    PA4     ------> TIM1_CH2N
    PB5     ------> TIM1_CH3N  */
    gpio_config.pin = GPIO_PIN_0 | GPIO_PIN_1 | GPIO_PIN_4;
