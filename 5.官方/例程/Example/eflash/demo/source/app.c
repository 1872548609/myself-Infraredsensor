...
void eflash_page_check_byte(uint16_t base_addr, uint8_t start_page, uint8_t num_page)
{
    ...
    {
        eflash_erase_page(base_addr + page * PAGE_SIZE);								//页擦除
        for(i = 0; i < PAGE_SIZE; i++)
        {
...
//        eflash_page_check_8bits(base_addr,0x55,page);                    //单字节读写
    }
...
void eflash_page_check_bytes(uint16_t base_addr,uint8_t w_val, uint32_t length)
{
    ...
    //多字节读写测试
    eflash_erase_page(base_addr);								//页擦除
    ...
    eflash_write_bytes(base_addr, write_data, length);                       //多字节写数据
        
   if(length == eflash_read_bytes(base_addr, read_data, length))             //多字节读    
   {
			for(i=0;i<length;i++)
			{
...
void eflash_sn_test(void)
{
  ...
  printfS("--------read chip Vcap for 4 bytes-------\n");
  read_Vcap(chip_Vcap);
	printfS("Vcap(4bytes): \n");
	printfB8(chip_Vcap,4);
	printfS("--------end-------\n\n");
...
void eflash_test(void)
{
	...
	eflash_init(system_core_clock);
	
	/********MAIN区擦读写测试*********/
    //单字节读写测试
	printfS("0x2000- 0x2200 write and read by byte test start\r\n");
	eflash_page_check_byte(0x2000,0,1);
	printfS("0x2000- 0x2200 write and read by byte test end\r\n"); 
    
	delay1ms(10);
    //多字节读写测试
	printfS("0x2000 write and read by multi bytes test start\r\n");
	eflash_page_check_bytes(0x2000,0x5a,20);                 //多字节读写
	
	printfS("0x2000 write and read by multi bytes test end\r\n");
	

    //SN(16 Bytes or 8 Bytes)
	eflash_sn_test();
