...
static volatile uint32_t s_period_accumulated_us = 0U;
...
static uint8_t GS358_IsTotalPeriodValid(uint32_t total_period_us);
...
static uint8_t GS358_IsTotalPeriodValid(uint32_t total_period_us)
{
    uint32_t target_total_us;
    uint32_t minimum_total_us;
    uint32_t maximum_total_us;

    target_total_us =
        (uint32_t)GS358_SIGNAL_PERIOD_US *
        (uint32_t)GS358_PERIOD_CHECK_COUNT;

    if (target_total_us >
        (uint32_t)GS358_TOTAL_PERIOD_TOLERANCE_US)
    {
        minimum_total_us =
            target_total_us -
            (uint32_t)GS358_TOTAL_PERIOD_TOLERANCE_US;
    ...
    if ((total_period_us >= minimum_total_us) &&
        (total_period_us <= maximum_total_us))
    {
        return 1U;
    }
...
void GS358_ComparatorFallingIRQHandler(void)
{
    ...
    if (period_valid != 0U)
    {
        ...
        if (s_edge_confirm_count < GS358_PERIOD_CHECK_COUNT)
        {
     ...
     *
     * 1. 单周期有效数量达到7次；
     * 2. 10个间隔累计时长符合10倍标准周期。
     */
    if (s_period_sample_count >= GS358_PERIOD_CHECK_COUNT)
    {
        /*
         * 在清零当前统计窗口之前，保存本轮结算数据，
         * 方便通过Keil Watch持续观察上一轮结果。
         */
