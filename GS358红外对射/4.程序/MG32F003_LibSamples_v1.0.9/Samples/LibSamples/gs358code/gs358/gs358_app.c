 ...
 *
 * GS358_PERIOD_TIMER = TIM3，用于连续100 us周期预测。
 * TIM3更新中断打开窗口，CC1中断关闭并结算窗口。
 */
...
volatile uint8_t  g_gs358_window_open = 0U;
volatile uint8_t  g_gs358_window_edge_count = 0U;
volatile uint8_t  g_gs358_last_window_edge_count = 0U;
volatile uint8_t  g_gs358_last_window_valid = 0U;
volatile uint8_t  g_gs358_sync_locked = 0U;
volatile uint8_t  g_gs358_valid_window_count = 0U;
volatile uint8_t  g_gs358_invalid_window_count = 0U;
volatile uint32_t g_gs358_window_total = 0U;
volatile uint32_t g_gs358_window_valid_total = 0U;
volatile uint32_t g_gs358_window_missing_total = 0U;
volatile uint32_t g_gs358_window_noise_total = 0U;
volatile uint32_t g_gs358_window_severe_noise_total = 0U;
...
static void GS358_PeriodTimerStartFromEdge(void);
static void GS358_PeriodTimerStop(void);
static void GS358_ResetWindowFilter(void);
static void GS358_FinishWindow(void);
...
void GS358_AppInit(void)
{
    ...
    g_gs358_window_open = 0U;
    g_gs358_window_edge_count = 0U;
    g_gs358_sync_locked = 0U;
    g_gs358_valid_window_count = 0U;
    g_gs358_invalid_window_count = 0U;
    g_gs358_window_total = 0U;
    g_gs358_window_valid_total = 0U;
    g_gs358_window_missing_total = 0U;
    g_gs358_window_noise_total = 0U;
    g_gs358_window_severe_noise_total = 0U;


    /*
     * 上电默认按“遮光/无光”状态输出。
     * TIM3此时不启动，收到第一条边沿后才建立同步。
     */
    GS358_ApplyOutputs();
...
/*
 * TIM3连续以1 MHz、GS358_SIGNAL_PERIOD_US为周期运行。
 *
 * 第一个边沿到来时把CNT设置为GS358_WINDOW_LATE_US：
 * 下一次更新事件恰好位于“下一预测边沿 - EARLY”处，打开窗口；
 * CCR1位于EARLY+LATE处，恰好在“预测边沿 + LATE”处关闭窗口。
 *
 * 定时器启动后连续运行，EXTI中的边沿不会重新清零计数器，
 * 因而窗口相位不会被窗口内或窗口外噪声拖动。
 */
static void GS358_PeriodTimerInit(void)
{
    ...
    timer_init.TIM_Period =
        (uint16_t)(GS358_SIGNAL_PERIOD_US - 1U);
    ...
    TIM_SetCompare1(
        GS358_PERIOD_TIMER,
        (uint16_t)(GS358_WINDOW_EARLY_US +
                   GS358_WINDOW_LATE_US));
...
static void GS358_PeriodTimerStartFromEdge(void)
{
    ...
    TIM_ClearITPendingBit(GS358_PERIOD_TIMER,
                          TIM_IT_Update | TIM_IT_CC1);
    NVIC_ClearPendingIRQ(TIM3_IRQn);
    TIM_SetCounter(GS358_PERIOD_TIMER,
                   (uint16_t)GS358_WINDOW_LATE_US);
...
static void GS358_FinishWindow(void)
{
    ...
    g_gs358_window_open = 0U;
    g_gs358_last_window_edge_count =
        g_gs358_window_edge_count;
    g_gs358_window_total++;
    ...
    if (g_gs358_last_window_edge_count == 1U)
    {
        g_gs358_last_window_valid = 1U;
        g_gs358_last_period_valid = 1U;
        g_gs358_last_period_us = GS358_SIGNAL_PERIOD_US;
        g_gs358_last_raw_period_us = GS358_SIGNAL_PERIOD_US;
        g_gs358_last_adjusted_period_us = GS358_SIGNAL_PERIOD_US;
        g_gs358_window_valid_total++;
        g_gs358_period_valid_total++;
        g_gs358_invalid_window_count = 0U;

        if (g_gs358_valid_window_count <
            GS358_LIGHT_CONFIRM_WINDOWS)
        {
            g_gs358_valid_window_count++;
        }
    ...
    {
        g_gs358_last_window_valid = 0U;
        g_gs358_last_period_valid = 0U;
        g_gs358_period_invalid_total++;
        g_gs358_valid_window_count = 0U;

        if (g_gs358_last_window_edge_count == 0U)
        {
            g_gs358_window_missing_total++;
            g_gs358_period_miss_total++;
        }
        else
        {
            g_gs358_window_noise_total++;

            if (g_gs358_last_window_edge_count >=
                GS358_SEVERE_NOISE_EDGE_COUNT)
            {
                g_gs358_window_severe_noise_total++;
            }
        }

        if (g_gs358_invalid_window_count < 255U)
        {
            g_gs358_invalid_window_count++;
        }

        invalid_limit =
            (g_gs358_light_present != 0U) ?
            GS358_LIGHT_LOST_WINDOWS :
            GS358_SYNC_LOST_WINDOWS;

        if (g_gs358_invalid_window_count >= invalid_limit)
        {
            GS358_PeriodTimerStop();
            GS358_SetLightState(0U);
            GS358_ResetWindowFilter();
        }
    }
...
void GS358_ComparatorFallingIRQHandler(void)
{
    ...
    /* 未同步时，第一条下降沿只建立100us预测时间基准。 */
    if (g_gs358_sync_locked == 0U)
    {
        GS358_ResetWindowFilter();
        g_gs358_sync_locked = 1U;
        GS358_PeriodTimerStartFromEdge();
        return;
    }

    /*
     * 同步后只有窗口内的边沿参与判定。
     * 窗口外噪声直接忽略，也绝不重新启动TIM3。
     */
    if (g_gs358_window_open != 0U)
    {
        if (g_gs358_window_edge_count < 255U)
        {
            g_gs358_window_edge_count++;
        }
    }
