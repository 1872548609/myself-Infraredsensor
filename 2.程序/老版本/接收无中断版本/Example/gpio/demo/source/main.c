...
/*
 * Timer0 配置为 100us 溢出一次。
 *
 * 计算方式：
 *  主频 24MHz；
 *  Timer 分频 16；
 *  Timer 计数频率 = 24MHz / 16 = 1.5MHz；
 *  1 个计数周期 = 0.666us；
 *  150 个计数约等于 100us。
 *
 * 注意：
 ...
 *  ADC 在 while(1) 里一直扫。
 */
#define TIMER0_100US_RELOAD             150U
 ...
 *  150 -> 15ms
 */
#define RX_LOST_TIMEOUT_100US           50U
 ...
 *  电位器变化很慢，50ms 更新一次足够。
 */
#define ADJ_UPDATE_PERIOD_100US         500U
 ...
 */
static volatile uint16_t g_tick_100us = 0;
...
static void timer0_init_poll_100us(void);
...
void main(void)
{
    ...
    /*
     * Timer0 初始化为 100us 轮询时间基准。
     *
     * 不打开 Timer0 中断。
     */
    timer0_init_poll_100us();
...
static void timer0_init_poll_100us(void)
{
    /*
     * 配置 Timer0：
     *  自动重装值 TIMER0_100US_RELOAD；
     *  预分频 TIMER0_PRESCALER。
     */
    gtimer0_count_init(TIMER0_100US_RELOAD, TIMER0_PRESCALER);
...
static void timer0_poll_process(void)
{
    ...
    if(REG_GTIM0_SR & GTIMER0_UIF)
    {
         ...
         */
        if(g_adj_update_tick >= ADJ_UPDATE_PERIOD_100US)
        {
            g_adj_update_tick = 0;
            threshold_update_process();
        }
...
static void valid_pulse_process(void)
{
     ...
     * 丢光判断就是靠这个时间做超时。
     */
    g_last_pulse_tick = g_tick_100us;
...
static void lost_timeout_process(void)
{
     ...
     */
    diff = (uint16_t)(g_tick_100us - g_last_pulse_tick);

    /*
     * 如果超过丢光时间，还没有新的有效脉冲，
     * 就认为遮光 / 丢光。
     */
    if(diff >= RX_LOST_TIMEOUT_100US)
    {
        /*
         * 清除连续有效脉冲计数。
         */
