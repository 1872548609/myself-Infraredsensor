...
{  		
	...
	gtimer0_count_init(100,16-1);		//gtimer0_count_init(16000,1000-1);		//时钟源为系统时钟16M，1000分频，100us溢出产生中断
	gtimer0_irq_init(GTIMER_IRQ_ENABLE,GTIMER0_UIE_IRQ,gtimer0_UECallBack);

	gtimer0_start();		//启动gtimer0计数	

	adc_clk_config(ADC_CLKSOURCE_SYSCLK, ADC_VREFSOURCE_AVDD33, 1, ADC_ENABLE);	//1.ADC时钟源选择 2.ADC电压基准源选择 3.ADC内部时钟分频倍数

	adc_sample_clk_config(ADC_SAMPCLK_4);										//采样时钟脉冲宽度配置
	
	adc_io_config(ADC_CHANNEL_1|ADC_CHANNEL_2);								//配置相应IO为ADC输入
...
//					adc_convert_start(ADC_CHANNEL_1);									//选择转换通道并启动转换

//					while((ADCGCR1 & 0x04) != 0);
//					
//					while(!(ADCCSTAT & 0x01))	;
//					
//					ADCCSTAT = 0x1;											//清除BUF状态位
//					
//					adc_sum+=adc_get_value();	//读值
//				
				
//					adc_convert_start(ADC_CHANNEL_2);									//选择转换通道并启动转换
//					
//					while((ADCGCR1 & 0x04) != 0);
//					
//					while(!(ADCCSTAT & 0x01))	;
...
//					ADCCSTAT = 0x1;											//清除BUF状态位
//					
//					adc_data1=adc_get_value();	//读值
//				
//					if(adc_data >=3500&&adc_data<4095)											
//					{
//								adc_set=3000;
//					} 
...
//				REG_GTIM0_CR0 &= ~(1<<0);				
				...
				adc_convert_start(ADC_CHANNEL_2);								
