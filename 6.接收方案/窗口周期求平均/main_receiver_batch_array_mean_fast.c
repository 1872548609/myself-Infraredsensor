 ...
 */
#define RX_CAPTURE_WINDOW_US            1100U

/*
 * 一帧最多保存多少个 ADC 点。
 * 旧版 384 点处理量偏大；本快速版限制为 160 点。
 * 采集窗口仍持续 1100us，但数组满后不再写入，后续处理只处理 160 点，
 * 可以显著缩短“窗口关闭到下一窗口打开”的处理间隔。
 */
#define RX_ADC_FRAME_MAX_SAMPLES        160U
 ...
 */
#define RX_SIGNAL_TOP_AVG_COUNT         4U

/*
 * 背景基线均值使用一帧里最弱的若干个点。
 * 因为一帧中绝大部分时间都没有红外脉冲，所以低端样本通常代表环境背景。
 */
#define RX_BASELINE_AVG_COUNT           8U
...
/* 连续多少帧无效后确认遮光。 */
#define RX_FRAME_OFF_CONFIRM_COUNT      4U
...
/*
 * 处理时间优化第一项：关闭串口长文本调试。
 * printfS 是阻塞发送，115200bps 下一百多个字符就可能占用 8~15ms。
 * 需要看 ADC 数据时再临时改成 1U。
 */
...
static void rx_process_frame(void)
{
     ...
     *   1 次遍历找最低若干点；
     *   1 次遍历统计有效点。
     * 并且最高/最低均值函数内部还有 top_n/low_n 小循环。
     *
     * 本版把 min/max/mean、最高点均值、最低点均值合并到同一趟遍历里。
     * 后面只再遍历一次统计有效点。
     * 处理量明显下降，窗口关闭后的空白时间会缩短。
     */
    ...
    for(i = 0U; i < count; i++)
    {
        ...
        /* 高脉冲有效：最高若干点代表信号。 */
        if(signal_used < RX_SIGNAL_TOP_AVG_COUNT)
        {
            g_proc_signal_buf[signal_used] = v;
            signal_used++;
        }
        else
        {
            replace_index = 0U;
            for(j = 1U; j < RX_SIGNAL_TOP_AVG_COUNT; j++)
            {
                if(g_proc_signal_buf[j] < g_proc_signal_buf[replace_index])
                {
                    replace_index = j;
                }
            }

            if(v > g_proc_signal_buf[replace_index])
            {
                g_proc_signal_buf[replace_index] = v;
            }
        }

        /* 高脉冲有效：最低若干点代表背景。 */
        if(base_used < RX_BASELINE_AVG_COUNT)
        {
            g_proc_base_buf[base_used] = v;
            base_used++;
        }
        else
        {
            replace_index = 0U;
            for(j = 1U; j < RX_BASELINE_AVG_COUNT; j++)
            {
                if(g_proc_base_buf[j] > g_proc_base_buf[replace_index])
                {
                    replace_index = j;
                }
            }

            if(v < g_proc_base_buf[replace_index])
            {
                g_proc_base_buf[replace_index] = v;
            }
        }
#else
        /* 低脉冲有效：最低若干点代表信号。 */
    ...
    signal_sum = 0UL;
    for(j = 0U; j < signal_used; j++)
    {
        signal_sum += g_proc_signal_buf[j];
    }

    base_sum = 0UL;
    for(j = 0U; j < base_used; j++)
    {
        base_sum += g_proc_base_buf[j];
    }

    signal_mean = (signal_used != 0U) ? (uint16_t)(signal_sum / signal_used) : 0U;
    baseline = (base_used != 0U) ? (uint16_t)(base_sum / base_used) : 0U;
