...
#define THRESHOLD_MAX_ADC               800U   //2800U
 ...
 */
#define RX_OFF_MARGIN_ADC               1000U
 ...
 *  但容易被单个干扰脉冲误触发。
 */
#define RX_PULSE_CONFIRM_COUNT          1U
 ...
 *  打开串口周期性调试回传。
 *  保留原来的 S/A/TH/INV/VP/L/P 状态输出，
 *  额外增加 AMP 字段，用来显示本周期内捕捉到的有效脉冲最大幅度。
 *
 ...
 *  所以这里不是每个 ADC 点都打印，而是按周期汇总一次。
 */
#define UART_ADC_DEBUG_ENABLE           1U    /* 1=打开串口测试；0=关闭串口测试 */
 ...
 * 单位：ms。
 * 当前 100ms 输出一次。
 * AMP 表示本 100ms 周期内捕捉到的最大有效脉冲 ADC 幅度；
 * 如果本周期没有捕捉到有效脉冲，则 AMP=0。
 */
 ...
 * 本周期内最后一次有效脉冲 ADC 幅度。
 *
 * 这里的“有效”是指已经超过 signal_enter_active() 判断阈值的 ADC 值，
 * 不再记录两个脉冲之间的 0 或背景值。
 */
static uint16_t g_uart_valid_pulse_amp_last = 0;

/*
 * 本周期内有效脉冲 ADC 最大幅度。
 *
 * 串口默认回传这个值。
 * 原因是发射信号是窄脉冲，单次采样不一定正好采到峰顶，
 * 用一个回传周期内的最大值更方便判断实际接收强度。
 */
static uint16_t g_uart_valid_pulse_amp_max = 0;

/*
 * 本周期内采到的有效 ADC 点数。
 *
 * 只要这个值为 0，说明本周期没有捕捉到有效脉冲，串口不回传。
 */
static uint16_t g_uart_valid_sample_count = 0;
#endif
...
#if UART_ADC_DEBUG_ENABLE
static void uart_adc_debug_record_valid_signal(uint16_t signal);
...
void main(void)
{
     ...
     * 串口初始化。
     *
     * 只有打开 UART_ADC_DEBUG_ENABLE 时才初始化串口。
     * 关闭串口调试时，不调用 uart_init()，避免串口占用 IO 或引入额外耗时。
     *
     * 注意：
    ...
    while(1)
    {
         ...
         * 串口周期性回传完整状态。
         *
         * 由 UART_ADC_DEBUG_ENABLE 宏控制：
         *  1：打开 S/A/TH/INV/VP/L/P/AMP 调试输出；
         *  0：完全关闭串口测试，不调用 printfS。
         */
...
static void signal_fast_scan_process(void)
{
    ...
    if(signal_enter_active(signal))
    {
         ...
         *
         * 这里不会记录 S=0 或背景值；
         * 只有超过进入阈值的 ADC 值才进入串口调试缓存。
         */
        uart_adc_debug_record_valid_signal(signal);
#endif
...
static void uart_adc_debug_record_valid_signal(uint16_t signal)
{
     ...
     * 只记录已经超过有效阈值的 ADC 值。
     *
     * 这个函数只在 signal_enter_active(signal) 成立后调用，
     * 所以这里记录到的一定是有效脉冲区域内的幅度，
     * 不会记录无信号时的 0。
     */
    g_uart_valid_pulse_amp_last = signal;

    /*
     * 记录本回传周期内的最大有效脉冲幅度。
     *
     * 因为红外发射是窄脉冲，ADC 不一定每次都采到峰顶，
     * 取一个周期内最大值更利于判断接收强度。
     */
    if(signal > g_uart_valid_pulse_amp_max)
    {
        g_uart_valid_pulse_amp_max = signal;
    }

    /*
     * 统计本周期内采到过多少个有效 ADC 点。
     *
     * 串口发送时会用它判断“本周期是否真的有有效脉冲”。
     */
    if(g_uart_valid_sample_count < 65535U)
    {
        g_uart_valid_sample_count++;
    }
}
...
static void uart_adc_debug_process(void)
{
    ...
    /*
     * AMP：本周期内捕捉到的最大有效脉冲幅度。
     *
     * g_uart_valid_sample_count > 0：
     *  本周期至少有一次 ADC 采样超过有效阈值，AMP 输出最大值。
     *
     * g_uart_valid_sample_count == 0：
     *  本周期没有捕捉到有效脉冲，AMP 输出 0。
     *
     * 注意：
     *  S 仍然是“打印时刻最近一次 ADC 值”，所以它可能经常是 0；
     *  AMP 才是这个回传周期内真正捕捉到的有效脉冲峰值。
     */
    if(g_uart_valid_sample_count != 0)
    {
        amp = g_uart_valid_pulse_amp_max;
    }
    else
    {
        amp = 0;
    ...
    /*
     * 清空本周期有效脉冲统计，准备统计下一个周期。
     */
    g_uart_valid_pulse_amp_last = 0;
    g_uart_valid_pulse_amp_max = 0;
    g_uart_valid_sample_count = 0;

    /*
     * 周期性回传完整调试状态。
     *
     * S   ：最近一次接收信号 ADC 值，可能刚好在脉冲空白区，所以常见为 0；
     * A   ：ADJ 电位器 ADC；
     * TH  ：当前判断阈值；
     * INV ：连续无效 ADC 读取次数；
     * VP  ：连续有效脉冲确认计数；
     ...
     * P   ：当前是否处于有效脉冲内部；
     * AMP ：本周期内捕捉到的最大有效脉冲 ADC 幅度，没有有效脉冲则为 0。
     */
    printfS("S=%u,A=%u,TH=%u,INV=%u,VP=%u,L=%u,P=%u,AMP=%u\r\n",
            g_adc_signal,
            g_adc_adj,
            g_threshold,
            g_invalid_sample_count,
