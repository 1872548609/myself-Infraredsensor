 ...
 *  一个周期内如果没有有效脉冲，就不回传任何内容，避免再看到 S=0。
 */
 ...
 * 单位：ms。
 * 当前 100ms 检查一次。
 * 只有本周期内捕捉到有效脉冲，才回传该周期内的最大有效脉冲幅度；
 * 如果本周期内没有有效脉冲，不发送任何内容。
 */
...
static void uart_adc_debug_process(void)
{
    ...
    /*
     * 本周期内没有捕捉到有效脉冲，不回传任何内容。
     *
     * 这样串口上就不会再出现 S=0、背景值、INV 等无信号检测信息。
     */
     ...
     * 默认回传本周期内最大有效脉冲幅度。
     *
     * 如果你想看最后一次有效幅度，可以把下面这一行改成：
     *  amp = g_uart_valid_pulse_amp_last;
     */
